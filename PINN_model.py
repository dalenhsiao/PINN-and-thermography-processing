...
class PhysicsInformedNN:
    ...
    def __init__(self, X, X_f,lb,ub, xbc, ybc, zbc, layers,load_model = 0, ModelDir = ""):
        ...
        if load_model:
            self.layers, self.best_weights, self.best_biases, self.best_lambda_1 = self.load_NN(ModelDir)
        ...
        self.y_f_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.z_f_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.t_f_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])

        # BC 
        self.x_x_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.x_y_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.x_z_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.x_t_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])

        self.y_x_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.y_y_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.y_z_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.y_t_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])

        self.z_x_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.z_y_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.z_z_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.z_t_bc_tf = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])


        # u prediction
        self.u_pred, _, _, _ = self.net_u(self.x_tf, self.y_tf, self.z_tf, self.t_tf)
        # bc loss 1st derivative
        # The Bc is assumed to be 0 heat flux at the boundary
    ...
    def initialize_NN(self, layers):
        ...
        for l in range(0, num_layers - 1):
            ...
            weights.append(W)
            biases.append(b)
            self.numWeights += layers[l] + layers[l+1]
    ...
    def neural_net(self,X, weights, biases):
        ...
        b = biases[-1]
        # final output
        Y = tf.add(tf.matmul(H, W), b)      # Y = matmul(input, weights) + bias
        return Y
    ...
    def train(self, nIter, loss_array, lambda_array, fileName,early_stopping = 1000):
        ...
        print(f"Best_loss = {self.best_loss}; L1 = {self.best_lambda_1}; Iter {best_iter}")
        self.plot_training(loss_array[:it], fileName, self.best_loss)

    def predict(self, X_star):
        self.lb = np.array([0,0,0,0])
        self.ub = np.array([X_star[:,0:1].max(), X_star[:,1:2].max(), X_star[:,2:3].max(), X_star[:,3:4].max])
    ...
    def summary(self):
        print(f'NumLayers: {len(self.layers)}, total weights: {self.weights}')
